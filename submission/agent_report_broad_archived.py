...
def build(evidence):
    ...
    panels = data['panels']
    validation = json.loads((evidence / 'agent-e2e/independent-validation.json').read_text())
    if validation['status'] != 'passed' or validation['completeMatrixExecutionsChecked'] != sum(p['independentExecutions'] for p in panels.values()):
        raise RuntimeError('Independent verification of all completed coding runs is required')
...
'''
    ...
    for name, panel in [('主面板', main), ('Pro', pro), ('历史版本扩展', extension), ('第二模型', secondary)]:
        if panel:
            cost_rows.append(f"| {name} | {panel['independentExecutions']} | {panel['actualModelCny']-panel['unknownUsageUpperCny']:.2f} | {panel['unknownUsageUpperCny']:.2f} |")
...
| 面板 | 独立 Agent 执行 | 已知用量计价 / 元 | 未知用量保守上界 / 元 |
|---|---|---|---|
...
用户批准总预算从 200 元提高至 1,000 元，其中本轮共享账本上限 990 元、此前工作预留 10 元。此变更仅调整资金上限，未改变固定任务、四组、三次重复、单次步数和时间限制。

初期 Sphinx-2549 和 Pro Ansible 任务继承宿主 HTTP 代理，后续完整任务固定绕过显式 HTTP 代理；系统路由仍可能经过网络隧道。全部初期结果保留，并提供排除这两个传输组的敏感性结果，不能忽略网络稳定性对时间预算的影响。

API 价格、模型别名和远程服务可能变化；本次运行保存实际响应、参数和用量。公开镜像预检会触发上游已定义的依赖重建或环境修复。任务排除、仿真开销、记忆来源和有限的仓库覆盖共同限定本报告的外推范围。

评分适配器曾错误地把不可变镜像摘要当成临时标签，导致测试启动前失败。修复后保持原镜像内容，重放全部受影响的已保存补丁；原评分与变更登记一并保留，没有重新抽样或新增模型调用。汇总程序拒绝旧版评分和不可评分的环境异常。开发样本的 131 项测试以及正式 Requests-3359 参考补丁的 69 项测试均完成独立适配器校验。

数据与框架：[SWEContextBench](https://github.com/jiayuanz3/SWEContextBench)、[SWE-bench Pro](https://github.com/scaleapi/SWE-bench_Pro-os)、[mini-swe-agent](https://github.com/SWE-agent/mini-swe-agent)。最初实验源码为 69b24f8，正式调用前登记为 37206f2；评分修复 e2b323b、登记修订 822e32a，预算授权 e1072b5、登记修订 256c9f7。后续修订明确发生在模型调用后，完整记录见 registration.json 及 registrations 归档。
'''
