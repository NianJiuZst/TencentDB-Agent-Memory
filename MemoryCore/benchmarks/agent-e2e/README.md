...
- `pilot-30/selection.json`：按该顺序选择的前 30 个参考预检合格任务，同一修复 PR 只取一个。
- `pilot-30/references/`：只读取参考验证信息的纳入/排除决定，不访问 Agent 成败。
- `pilot-30/pairs/`：每个任务两组结果，以及配置相同的旧 MiniMax 运行复用记录。
- `pilot-30/runs/`、`formal/main-minimax/`：新执行与复用执行的真实轨迹、补丁和独立测试。
- `pilot-30/analysis.json`：独立复算、成对改善/退化、bootstrap 区间、精确 McNemar 检验、费用和记忆覆盖。

两组使用同一初始提交、工具和限制：80 步、步骤边界检查的 1,200 秒预算、每次输出 8,192 tokens、温度 0.2、adaptive thinking、1.5 CPU、2 GiB 内存。模型未提交补丁时计为预算内失败。60 次均是分别执行，不因上下文相同而共用结果。原有符合条件的主面板 MiniMax 第零次运行，不论成功或失败都复用。
