...
export interface TrajectoryExpansionPhaseAdmission {
...
export interface LongMemEvalV2TrajectoryExpansionBaselineCase {
...
export interface LongMemEvalV2TrajectoryExpansionBaselineSummary {
  protocolVersion: string;
  mode: "base";
  phase: LongMemEvalV2TrajectoryExpansionPhase;
  status: "completed";
...
export function assertTrajectoryExpansionPhaseReadAuthorized(params: {
  phase: LongMemEvalV2TrajectoryExpansionPhase;
  authorization?: TrajectoryExpansionPhaseAdmission;
}): void {
  if (params.phase === "development") {
    if (params.authorization) throw new Error("D12 development must not use a later-phase authorization");
    return;
  }
  ...
  if (!authorization
    || authorization.admissionVersion
      !== "lifecycle-longmemeval-v2-trajectory-expansion-admission-v1.0"
    ...
    throw new Error(`D12 ${params.phase} read requires a committed ${expectedCompleted}-passed admission`);
  }
}

export async function runLongMemEvalV2TrajectoryExpansionBaseline(params: {
  dataRoot: string;
  phase: LongMemEvalV2TrajectoryExpansionPhase;
  preScoreCommit: string;
  authorization?: TrajectoryExpansionPhaseAdmission;
  ...
  if (!/^[0-9a-f]{7,40}$/iu.test(params.preScoreCommit)) {
    throw new Error("D12 baseline preScoreCommit must be a git SHA");
  }
  ...
  if (params.phase !== "development"
    ...
    throw new Error(`D12 ${params.phase} baseline requires the admission artifact SHA-256`);
  }
  const protocol = LONGMEMEVAL_V2_TRAJECTORY_EXPANSION_PROTOCOL;
  ...
  const index: LongMemEvalV2TrajectoryExpansionBaselineSummary["index"] = {};
  try {
    ...
    const cases: LongMemEvalV2TrajectoryExpansionBaselineCase[] = [];
    for (const question of selectedQuestions) {
      const backend = backends.get(question.domain);
      if (!backend) throw new Error(`missing D12 Base backend ${question.domain}`);
