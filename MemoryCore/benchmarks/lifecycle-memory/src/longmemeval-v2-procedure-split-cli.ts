...
function metadataOnly(raw: RawQuestionMetadata): LongMemEvalV2ProcedureQuestionMetadata {
...
const { values } = parseArgs({
  options: {
    ...
    seed: { type: "string", default: "d9-procedure-20260822-v1" },
    output: { type: "string" },
  },
});
if (!values.data || !values.revision || !values.output) {
  throw new Error(
    "usage: tsx longmemeval-v2-procedure-split-cli.ts --data <root>"
