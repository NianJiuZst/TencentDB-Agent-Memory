...
| D11 | Query-relevant evidence-preserving substitution | Preserve bounded body evidence inside a same-trajectory capsule | D10 consumed diagnostics; D9 test remains unread | Research and protocol design | Prevent evidence loss without restoring monolithic raw context or weakening exact fallback |
...
D11 changes only this representation boundary. It will investigate a bounded, label-free query-relevant evidence layer drawn exclusively from the exact Base items already selected for replacement. Same-trajectory ownership, unrelated-item preservation, provenance, per-query token and item ceilings, complexity bounds, and exact Base fallback remain hard invariants. The next protocol must distinguish the contribution of body-evidence preservation from local-step feedback and cannot claim confirmation from the consumed audit.
