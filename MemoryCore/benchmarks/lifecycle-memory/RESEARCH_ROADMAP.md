...
On the 76 explicitly consumed D10 questions, the selected compact representation accepted 53 substitutions, reduced mean tokens by 22.74%, preserved the three D10 improvements, and routed the former `priority` harm to a capsule that retained the decisive body sentence. The exploratory direct-support estimate was +6.79 points with zero harms. A preceding verbose rendering accepted 51 substitutions with the same quality estimate; both iterations are recorded in the design artifact. These are design-selection observations, not new evidence. The next step is to freeze the protocol and implementation before recomputing the consumed audit, independently reproduce every certificate and gate, and read the 30-question test only if a committed admission exists.
